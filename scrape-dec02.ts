...
async function main() {
  ...
  try {
    ...
    console.log(`\nScraping complete: ${results.liensFound} liens found\n`);

    // Step 2: Check what we have in database from today's scrape
    console.log('--- STEP 2: VERIFYING DATABASE RECORDS ---\n');
    
    // Get liens scraped today (Dec 2)
    const todayLiens = await db.select().from(liens)
      .where(sql`${liens.recordDate} >= '2025-12-02' AND ${liens.recordDate} < '2025-12-03'`);
    
    console.log(`Liens in database from today: ${todayLiens.length}`);

    // Step 3: Verify PDFs
    console.log('\n--- STEP 3: VERIFYING PDFs ---\n');
    
    const liensWithValidPdfs: typeof todayLiens = [];
    const liensWithoutPdfs: typeof todayLiens = [];
    
    for (const lien of todayLiens) {
    ...
    console.log(`${'='.repeat(60)}`);
    console.log(`Total liens scraped: ${results.liensFound}`);
    console.log(`With valid PDFs: ${liensWithValidPdfs.length}`);
    console.log(`Synced to Airtable: ${syncedCount}`);
    console.log(`Failed: ${failedCount}`);
    console.log(`Success rate: ${((syncedCount / liensWithValidPdfs.length) * 100).toFixed(1)}%`);
    console.log(`${'='.repeat(60)}\n`);
